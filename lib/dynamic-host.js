...
return {
  ...
  apply(ctx) {
    ...
    const MARKER_ID_PREFIX = 'message-editor'
/** One in-flight op per session; later ops wait for the earlier one. */
  ...
  /** A single-node span: recall targets exactly one message, nothing else. */
  function singleNodeSpan(session, seq) {
    if (!session.surface.nodes.includes(seq)) return null
    return {
      start: seq,
      end: seq,
      shadowedSeqs: [seq],
  ...
  function roundSpanFrom(session, seq) {
    ...
    let startIdx = index
    if (targetEvent?.type !== 'user/message') {
