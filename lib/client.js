...
const en = {
  ...
  'badge.recalled': 'Recalled',
  'badge.edited': 'Edited',
  'badge.regenerated': 'Regenerated',
...
/** True when `seq` was shadowed by any recall/edit/regenerate marker; returns the marker op. */
function useShadowInfo(useSession, seq) {
  return useSession((snapshot) => {
    ...
    for (const node of snapshot.chat.nodes.values()) {
      if (node.kind === 'recall-marker' && Array.isArray(node.data?.shadowedSeqs)
        ...
        return op === 'recall' ? 'recall' : op === 'regenerate' ? 'regenerate' : 'edit'
      }
    }
    return null
  })
...
function AssistantActions({ messageId, sessionId, useSession, t }) {
  ...
  const shadowInfo = useShadowInfo(useSession, seq)
  const [busy, setBusy] = useState(false)
  const [failure, setFailure] = useState(null)
  if (shadowInfo !== null) {
    return createElement('span', { className: 'dsh-me-badge', role: 'status' }, t('badge.recalled'))
  }
...
function UserActionsRow({ node, sessionId, useSession, t }) {
  ...
  const [failure, setFailure] = useState(null)
  if (shadowInfo !== null) {
    const label = shadowInfo === 'recall'
      ? t('badge.recalled')
      : shadowInfo === 'regenerate' ? t('badge.regenerated') : t('badge.edited')
    return createElement('div', { className: 'dsh-me-user-row' },
      createElement('span', { className: 'dsh-me-badge', role: 'status' }, label))
  }
...
.dsh-me-badge{color:var(--dsw-alias-label-tertiary);background:var(--dsw-alias-interactive-bg-hover);border-radius:10px;padding:0 8px;font-size:12px;line-height:20px;white-space:nowrap}
