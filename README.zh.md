...
| 操作 | 入口 | 效果 |
| --- | --- | --- |
| **↩ 撤回** | 悬停任意助手回复；或用户消息下方的操作行 | 该消息及其后的内容从模型上下文**和对话视图**中同步清除，并把原提问**回显到输入框**，方便立即修改后重发。 |
| **✎ 编辑重发** | 用户消息下方的操作行 | 回退并隐藏旧消息及其回复。默认**从新对话开始**（此前的消息一并隐藏、不再进入上下文），发送修改后的文本让智能体作答；可配置显示「原提问」对照块。 |
...
- **编辑后显示原提问对照**（默认开）：编辑后显示原始提问文本，方便对照。
